    ...
    el.innerHTML = `
      ...
      <div class="note-card" style="margin-bottom:12px">
        ...
        <div style="font-size:12px;margin-top:4px;line-height:1.8">
          ${plan.validation.passed && !plan.validation.warnings.length ? ' 全部达标！' : ' 部分未达标'}
          ...
          ${plan.validation.stats ? `
            <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:4px">
              <span> 食材 ${plan.validation.stats.weekDiversity?.count || 0}种/周 ${plan.validation.stats.weekDiversity?.passed ? '' : ''}</span>
              <span> 深色蔬菜 ${plan.validation.stats.darkVegetable?.ratioText || '—'} ${plan.validation.stats.darkVegetable?.passed ? '' : ''}</span>
              <span> 红肉 ${plan.validation.stats.redMeat?.total || 0}g/周 ${plan.validation.stats.redMeat?.passed ? '' : ''}</span>
              <span> 鱼虾 ${plan.validation.stats.fish?.count || 0}次/周 ${plan.validation.stats.fish?.passed ? '' : ''}</span>
            </div>` : ''}
        </div>
      </div>` : ''}
        ...
        <div id="plan-body-${idx}">
          ...
          <div class="ingredient-bar">
            ...
            <div style="display:flex;align-items:center;gap:8px">
              <span class="count">${Helpers.disp(day.ingredientCount, 0)}/12 ${day.ingredientCount >= 12 ? '' : ''}</span>
