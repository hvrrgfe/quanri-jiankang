...
const pickNew = `    this._answers[this._currentQ] = optionIdx;
    // Save progress
    try { Store.set('psy_progress', { key: this._currentCat + '_' + this._currentKey, answers: this._answers, currentQ: this._currentQ + 1, elapsed: Date.now() - (this._testStart || Date.now()) }); } catch(e) {}
    this._renderQ('next');`;
if (c.includes(pickOld)) { c = c.replace(pickOld, pickNew); console.log('1. Save progress in _pick'); }
...
const startNew = `  _start(cat, key) {
    ...
    if (savedProgress.key === cat + '_' + key && savedProgress.answers && savedProgress.currentQ > 0) {
      if (confirm('\\u4e0a\\u6b21\\u6d4b\\u8bd5\\u8fdb\\u5ea6\\u672a\\u5b8c\\u6210\\uff0c\\u662f\\u5426\\u7ee7\\u7eed\\uff1f')) {
        this._currentQ = savedProgress.currentQ; this._answers = savedProgress.answers;
        this._testStart = Date.now() - (savedProgress.elapsed || 0); this._renderQ('next'); return;
      }
    ...
    this._answers = {};
    this._renderIntro();`;
if (c.includes(startOld)) { c = c.replace(startOld, startNew); console.log('2. Restore progress in _start'); }

// === Fix 3: Clear progress on result ===
const resultOld = 'p.psyAssessments[this._currentKey] = {';
const resultNew = 'Store.remove(\\'psy_progress\\'); p.psyAssessments[this._currentKey] = {';
if (c.includes(resultOld)) { c = c.replace(resultOld, resultNew); console.log('3. Clear progress on result'); }

// === Fix 4: Add answer timing ===
ansOld = "this._answers[this._currentQ] = optionIdx;\\n    this._renderQ('next');"
// Already handled in Fix 1, so add timing to the pick method

// === Fix 5: Replace _genAIAnalysis method ===
const genAIOld = `  _genAIAnalysis() {
    ...
    var prompt = '\u91cf\\u8868:' + scale.name + '(' + scale.items.length + '\u9898) \u5f97\\u5206:' + totalScore + '/' + maxScore +
      ...
      ' \u56de\u7b54:' + answers.join('|') +
      ' \u8bf7\\u6309\u7ed3\u6784\u8f93\\u51fa:\u3010\u603b\u4f53\u89e3\u8bfb\u3011\u3010\u7ef4\u5ea6\u5206\u6790\u3011\u3010\u5efa\u8bae\u3011\u3010\u6ce8\u610f\u4e8b\u9879\u3011\u7eaf\\u6587\u672c\u6bcf\u6bb52-3\u884c';

    Helpers.callLLM('\u4f60\\u662f\u4e00\u4f4d\u4e34\u5e8a\u5fc3\u7406\u5b66\u4e13\u5bb6\u3002\u5206\u6790\u5ba2\u6237\u7684\u5fc3\u7406\u6d4b\u8bc4\u7ed3\u679c\uff0c\u8f93\u51fa\u7b80\u6d01\u4e13\u4e1a\u7684\u6587\u5b57\u62a5\u544a\u3002\u4f7f\u7528\u3010\u3011\u6807\u6ce8\u6bb5\u843d\u6807\u9898\u3002', prompt, Store.getApiKey()).then(function(result) {
      var text = '';
...
const genAINew = `  _genAIAnalysis() {
    ...
    if (isMbti) {
      ...
      var prompt = '\\u8fd9\\u662f\\u4e00\\u4e2a\\u5927\\u4e94\\u4eba\\u683c(IPIP-NEO-300)\\u6d4b\\u8bc4\\u7ed3\\u679c\\u3002\\u7c7b\\u578b:' + (typeLabel || '') + '\\n\\u7ef4\\u5ea6:' + dimData + '\\n' + topFacets + normData + '\\n\\n\\u8bf7\\u6309\\u4ee5\\u4e0b\\u7ed3\\u6784\\u8f93\\u51fa\\u5206\\u6790\\u62a5\\u544a\\uff08\\u6bcf\\u6bb52-4\\u884c\\uff09:\\n\\u3010\\u4eba\\u683c\\u753b\\u50cf\\u3011\\u3010\\u6838\\u5fc3\\u52a8\\u529b\\u3011\\u3010\\u6f5c\\u5728\\u76f2\\u533a\\u3011\\u3010\\u4eba\\u9645\\u6a21\\u5f0f\\u3011\\u3010\\u6210\\u957f\\u8def\\u5f84\\u3011';
      Helpers.callLLM('\\u4f60\\u662f\\u4e00\\u4f4d\\u4eba\\u683c\\u5fc3\\u7406\\u5b66\\u4e13\\u5bb6\\u3002\\u6839\\u636e\\u5927\\u4e94\\u4eba\\u683c\\u6d4b\\u8bc4\\u6570\\u636e\\uff0c\\u5bf9\\u6765\\u8bbf\\u8005\\u8fdb\\u884c\\u4e13\\u4e1a\\u3001\\u7cbe\\u51c6\\u3001\\u6709\\u6e29\\u5ea6\\u7684\\u4eba\\u683c\\u5206\\u6790\\u3002\\u4e0d\\u8981\\u6a21\\u7cca\\u7a7a\\u6cdb\\u7684\\u63cf\\u8ff0\\uff0c\\u8981\\u57fa\\u4e8e\\u5177\\u4f53\\u6570\\u636e\\u3002', prompt, Store.getApiKey()).then(function(result) {
      ...
      }).catch(function() { var c2 = document.getElementById('ai-psy-analysis'); if (c2) c2.innerHTML = '<div style="font-size:13px;color:var(--red)">\\u5206\\u6790\\u751f\\u6210\\u5931\\u8d25\\uff0c\\u8bf7\\u91cd\\u8bd5</div>'; });
    } else {
      ...
      var prompt = '\\u91cf\\u8868:' + scale.name + '(' + scale.items.length + '\\u9898) \\u5f97\\u5206:' + totalScore + '/' + maxScore + '(' + Math.round(totalScore/maxScore*100) + '%) \\u6807\\u51c6:' + (scale.scoring || '') + ' \\u56de\\u7b54:' + answers.join('|') + ' \\u8bf7\\u6309\\u7ed3\\u6784\\u8f93\\u51fa:\\u3010\\u603b\\u4f53\\u89e3\\u8bfb\\u3011\\u3010\\u7ef4\\u5ea6\\u5206\\u6790\\u3011\\u3010\\u5efa\\u8bae\\u3011\\u3010\\u6ce8\\u610f\\u4e8b\\u9879\\u3011\\u6bcf\\u6bb52-3\\u884c';
  ...
  _genAIChat() {
    ...
    msgs.innerHTML += '<div style="text-align:right;margin-bottom:4px"><span style="display:inline-block;padding:4px 10px;background:var(--brand-bg);border-radius:10px 10px 2px 10px;font-size:12px">' + question + '</span></div><div style="text-align:left;margin-bottom:4px" id="ai-chat-loading"><span style="display:inline-block;padding:4px 10px;background:var(--card);border-radius:10px 10px 10px 2px;font-size:12px;color:var(--text-hint)">\\u601d\\u8003\\u4e2d...</span></div>';
    msgs.scrollTop = msgs.scrollHeight;
    var ctx = this._aiContext || {};
    var base = '\\u4ee5\\u4e0b\\u662f\\u6b64\\u4eba\\u7684\\u5b8c\\u6574\\u4eba\\u683c\\u6d4b\\u8bc4\\u6570\\u636e\\u548c\\u5206\\u6790\\u62a5\\u544a\\u3002\\u8bf7\\u57fa\\u4e8e\\u6570\\u636e\\u56de\\u7b54\\uff0c\\u4e0d\\u8981\\u7f16\\u9020\\u3002';
    if (ctx.prompt) base += '\\n\\n\\u6570\\u636e:\\n' + ctx.prompt; if (ctx.result) base += '\\n\\n\\u5206\\u6790:\\n' + ctx.result;
    Helpers.callLLM(base, '\\u7528\\u6237\\u95ee\\u9898:' + question, Store.getApiKey()).then(function(result) {
      var text = ''; if (typeof result === 'object' && result.text) text = result.text; else if (typeof result === 'object' && result.content) text = result.content; else if (typeof result === 'string') text = result; else text = JSON.stringify(result);
      var loading = document.getElementById('ai-chat-loading'); if (loading) loading.outerHTML = '<div style="text-align:left;margin-bottom:4px"><span style="display:inline-block;padding:4px 10px;background:var(--card);border-radius:10px 10px 10px 2px;font-size:12px">' + text.substring(0, 300) + '</span></div>';
      var m = document.getElementById('ai-chat-msgs'); if (m) m.scrollTop = m.scrollHeight;
    }).catch(function() { var loading = document.getElementById('ai-chat-loading'); if (loading) loading.outerHTML = '<div style="text-align:left;margin-bottom:4px"><span style="display:inline-block;padding:4px 10px;background:var(--card);border-radius:10px 10px 10px 2px;font-size:12px;color:var(--red)">\\u5931\\u8d25</span></div>'; });
  },

  _getScale`;

if (c.includes(genAIOld)) {
  c = c.replace(genAIOld, genAINew);
  console.log('5. Replaced _genAIAnalysis + added chat methods');
} else {
  console.log('ERROR: _genAIAnalysis pattern not found');
  const idx = c.indexOf('_genAIAnalysis()');
  if (idx >= 0) console.log('Found at', idx, ':', c.substring(idx, idx + 200));
}

// === Fix 6: Lie scale + fatigue in validity ===
const validOld = "var consistencyIssues = 0;";
const validNew = "var consistencyIssues = 0;\n        var lieScore = 0;\n        var lieItems = [54,55,56,57,58,59];\n        lieItems.forEach(function(li) { var la = this._answers[li]; if (la !== undefined) { var isRev = li >= 55; var lv = isRev ? (la === 0 ? 1 : la === 4 ? 0 : la === 1 ? 0.5 : 0) : (la === 4 ? 1 : la === 0 ? 0 : la === 3 ? 0.5 : 0); lieScore += lv; } }.bind(this));\n        var lieFlag = lieScore >= 4 ? 1 : 0;\n        if (lieFlag) consistencyIssues++;\n        var fatigueFlag = 0;\n        if (this._answerTimes && this._answerTimes.length >= 80) { var headTimes = this._answerTimes.slice(0, 30).reduce(function(s,v) { return s+v; }, 0) / 30; var tailTimes = this._answerTimes.slice(-30).reduce(function(s,v) { return s+v; }, 0) / 30; if (tailTimes > 0 && headTimes / tailTimes > 2.0) { fatigueFlag = 1; consistencyIssues++; } }";
if (c.includes(validOld)) { c = c.replace(validOld, validNew); console.log('6. Added lie+fatigue to validity'); }

// === Fix 7: Add lie/fatigue text ===
const detailText = "validityDetail.push('可能存在社会赞许偏差');\n        if (fatigueFlag) validityDetail.push('尾段答题过快·可能疲劳')";
const detailOld = "validityDetail.push('\\u7b54\\u9898\\u8fc7\\u5feb\\uff08' + elapsed + '\\u5206\\u949f\\uff09')";
const detailNew = "validityDetail.push('\\u7b54\\u9898\\u8fc7\\u5feb\\uff08' + elapsed + '\\u5206\\u949f\\uff09')\\n        if (lieFlag) " + detailText;
if (c.includes(detailOld)) { c = c.replace(detailOld, detailNew); console.log('7. Added lie/fatigue text'); }
